...
export default function OrderInfo({ user = false }) {
  ...
  const ingrsToRender = ingrs?.reduce((acc, item) => {
  ...
  return (
    <div className={orderInfoStyles.main}>
      ...
      <p className={`${orderInfoStyles.done} text text_type_main-default pt-3`}>{types[currentOrder?.status]}</p>
      <p className={`text text_type_main-medium pt-15 pb-6`}>Состав:</p>
      <ul className={orderInfoStyles.ingrsContainer}>
        {Boolean(ingrsToRender) &&
